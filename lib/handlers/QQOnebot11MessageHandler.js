...
const CQHttp = require('cqhttp-lilywhitebot');
...
class QQOnebot11MessageHandler extends MessageHandler {
    constructor(config = {}) {
        super();
        ...
        client('get_version_info').then((json) => {
            winston.info(`QQBot (OneBot 11) Get OneBot Information: ${json.app_name} ${json.app_version} [OneBot ${json.protocol_version}]`);
            if (!`${json.protocol_version}`.includes("11")) {
                winston.warn(`<QQBot> OneBot version mismatch, functionality may be impacted.`);
            }
        }).catch(e => {
            winston.error(`QQBot (OneBot 11) Get OneBot Information Failed: `, e);
        });
        ...
        client.on('message', async (rawdata) => {
            ...
            if (this._multiMsgId) {
                if (parsedMsg.extra.multimsg) {
                    context.text+=`\n[私聊机器人使用 !qmulti ${parsedMsg.extra.multimsg[0]} 以${parsedMsg.extra.multimsg[1]}]`;
                } else if (parsedMsg.extra.forward) {
                    context.text+=`\n[私聊机器人使用 !qmulti ${parsedMsg.extra.forward[0]} 以查看转发消息]`;
                }
            }
            ...
            for (let video of parsedMsg.extra.videos) {
                ...
                // onebot在消息上報使用array格式時會有url值，不用去下載影片文件
            ...
            if (parsedMsg.extra.reply) {
              ...
              try {
                let data = await this._client('get_msg', {
                ...
                winston.error(`Retcode ${error.retcode} when getting reply msg: ${JSON.stringify(error, null, 2)}`);
              }
            }
    ...
    async getForwardMsg(id) {
        let retcode, status, error, data;
        try {
            data = await this._client('get_forward_msg', {
                message_id: id,
                id,
            });
            status = "ok";
        ...
        return {retcode, status, error, data};
    }
    ...
    async start() {
        if (!this._started) {
            ...
            winston.info(`QQBot (OneBot 11) is listening at ${this._host}:${this._port}...`);
        }
    }
...
module.exports = QQOnebot11MessageHandler;
