...
class QQOICQMessageHandler extends MessageHandler {
    constructor(config = {}) {
        ...
        let client = oicq.createClient(botConfig.qq, {
            ...
            sign_api_addr: botConfig.signApiAddr,
        });
        ...
        this._reconnInterval = botConfig.reconnInterval===undefined ? 5 : botConfig.reconnInterval,
        this._autoServer = botConfig.autoServer===undefined ? true : botConfig.autoServer,
        this._signApiAddr = botConfig.signApiAddr;
            ...
            switch (context.sub_type) {
                ...
                case 'decrease':
                    if (!context.dismiss) {
                      // 退群或被踢
                      let admin = null;
                      if (context.operator_id!=0 && context.operator_id!=context.user_id) {
                        try {
                          admin = await this.groupMemberInfo(context.group_id, context.operator_id);
                        } catch (_) {
                          admin = await this.strangerInfo(context.operator_id);
                        }
                      }
                      this.emit('leave', {
                          group: context.group_id,
        ...
        client.on('system.login', (info)=>{
            switch (info.sub_type) {
                case 'qrcode':
                    winston.info('QQBot QR code required.');
                    // fs.writeFileSync(`./data/qrcode_${Date.now()}.png`, info.image)
                    winston.info(`Please open ./data/${this._qq}/qrcode.png to scan the qr code.`);
                    break;
                case 'slider':
                ...
                case 'device':
                    ...
                    break;
                case 'error':
                    winston.error(`QQBot login error ${info.code}: ${info.message}`);
                    break;
            }
            this.stop();
        });
    ...
    async getForwardMsg(id) {
        return await this._client.getForwardMsg(id);
    }
    ...
    async start() {
        if (!this._started) {
            this._started = true;
            if (this._passwordMd5?.length) this._client.login(this._passwordMd5);
            else this._client.login();
        }
    }
